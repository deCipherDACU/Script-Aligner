...
import { alignSrtOffline, correctSrtWithWordBank } from './services/offlineService';
...
const App: React.FC = () => {
  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      if (scriptContent) {
        // Full alignment with script reference
        result = await alignSrtOffline(srtContent, scriptContent, customDictionary);
        showToast('Full alignment completed!', 'success');
      } else {
        // Word Bank only correction (no script)
        result = await correctSrtWithWordBank(srtContent, customDictionary);
        showToast('Word Bank corrections applied!', 'success');
      }
  ...
  const handleDownload = async () => {
    ...
    const finalFileName = `corrected_${nameWithoutExt.replace(/\s+/g, '')}.srt`;
        ...
        <div className="w-full md:w-[350px] p-6 flex flex-col border-b md:border-b-0 md:border-r border-[#383838] bg-[#222]">
          ...
          <div className="mb-6 space-y-2">
            <div className="flex justify-between items-end">
              <label className="text-xs font-bold text-gray-400 uppercase tracking-wider">1. Load Source (File or Paste)</label>
            </div>
            <Dropzone
          ...
          <div className="flex-1 flex flex-col mb-6 space-y-2 min-h-[200px]">
            <div className="flex justify-between items-center">
              <label className="text-xs font-bold text-gray-400 uppercase tracking-wider">
                2. Reference Script <span className="text-gray-600 font-normal">(Optional - improves accuracy)</span>
              </label>
          ...
          >
            {status === 'ALIGNING'
              ? 'Processing...'
              : scriptContent
                ? 'Full Align (with Script)'
                : 'Quick Fix (Word Bank Only)'
            }
          </Button>

          {!scriptContent && srtContent && (
            <p className="text-[10px] text-center text-gray-500 mt-2">
              Add a reference script for higher accuracy alignment
            </p>
          )}
